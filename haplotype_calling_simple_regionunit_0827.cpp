...
hash_map<string, hash_map<int, hash_map<int, vector<haplotype>>>> haplo_hashmap; // hash_map<chromosome, hash_map<start_position, hash_map<cpg_length, vector<haplotype>>>>
...
void haplotypeFormat(int id, const std::string & inputFileName) // id: file id for the further accumulation
{
    ...
    while (!haploFile.eof())
    {
        ...
        {
            hash_map<int, hash_map<int, vector<haplotype>>> newchr_map;
            haplo_hashmap[chr] = newchr_map;
        }
        if (!haplo_hashmap[chr].count(start))
        {
            hash_map<int, vector<haplotype>> newstart_map;
...
void distanceCompute(vector<string> input_file_names, string output_file_name)
{
    ...
    hash_map<string, hash_map<int, hash_map<int, vector<haplotype>>>>::iterator chr_iter;
    hash_map<int, hash_map<int, vector<haplotype>>>::iterator start_index_iter;
    hash_map<int, vector<haplotype>>::iterator haplo_length_iter;

    for (chr_iter = haplo_hashmap.begin(); chr_iter != haplo_hashmap.end(); chr_iter++)
    {
        string chr = chr_iter->first;
        hash_map<int, hash_map<int, vector<haplotype>>> haplo_map_chr = chr_iter->second;
        for (start_index_iter = haplo_map_chr.begin(); start_index_iter != haplo_map_chr.end(); start_index_iter++ )
        {
            ...
            hash_map<int, vector<haplotype>> haplo_map_start = start_index_iter->second;
